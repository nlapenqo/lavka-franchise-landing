...
try {
  ...
  for (const t of times) {
    ...
    if (probe.length) { const r = await send('Runtime.evaluate', { expression: `JSON.stringify(${JSON.stringify(probe)}.map(s => { const cs = getComputedStyle(document.querySelector(s)); const m = new DOMMatrix(cs.transform === 'none' ? undefined : cs.transform); return [s, +cs.opacity, m.m41]; }))`, returnByValue: true }); console.log(`t=${t.toFixed(2)} ` + JSON.parse(r.result.value).map(([s, o, tx]) => `${s}=${o.toFixed(2)}${tx ? ` tx=${tx.toFixed(1)}` : ''}`).join(' ')); }
