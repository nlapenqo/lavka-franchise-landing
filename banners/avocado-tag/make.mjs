...
const VER = '20260915c'; // версия banner.css в ссылке: Pages кэширует CSS на 10 минут, при правке стилей поднимать
...
/* ───── 3. Маятник: бирка качается на узелке, затухая, и повисает на черенке ───── */
{
  ...
  const css = [
    ...
    reveal('.tag__ring', 'v3ring', 1.25, 0.15, 'transform:scale(.4)', 'transform:scale(1)', BACK),
  ...
  variants.push({ n: 3, title: 'Маятник', desc: 'Ось вращения — узелок. Бирка появляется крупно без колечка и верёвочки, качнувшись, летит к авокадо; во время полёта из колечка вырастает верёвочка, на посадке ставится узелок — бирка повисает, затухающе покачиваясь. Контент выходит лесенкой.', css });
}
