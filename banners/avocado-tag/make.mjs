...
const VER = '20260915g'; // версия banner.css в ссылке: Pages кэширует CSS на 10 минут, при правке стилей поднимать
...
/* ───── 3. Маятник: бирка качается на узелке, затухая, и повисает на черенке ───── */
{
  ...
  const css = [
    ...
    reveal('.star', 'v3star', 1.0, 0.45, tf('star', 'scale(0)'), tf('star', 'scale(1)')),
    anim('.strike', 'v3strike', [
      { t: 0,    css: 'clip-path:inset(0 100% 0 0)' },
      { t: 1.35, css: 'clip-path:inset(0 100% 0 0)', ease: SOFT },
      { t: 1.6,  css: 'clip-path:inset(0 0 0 0)' },
    ]),
  ].join('\n');
  variants.push({ n: 3, title: 'Маятник', desc: 'Ось вращения — узелок. Бирка появляется крупно с колечком, качнувшись, летит к авокадо; авокадо проявляется ещё во время качания, в полёте из точки в колечке вырастает верёвочка, на посадке ставится узелок; контент выходит лесенкой прямо во время полёта.', css });
}
