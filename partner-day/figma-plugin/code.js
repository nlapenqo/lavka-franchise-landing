/* ============================================================================
   ...
   Плагин строит нативный макет: фреймы с auto-layout, текстовые слои
   на YS Geo / YS Text, фирменные цвета и радиусы Лавки.

   Эталон — 1920 (на боевой странице при ≥1920 rem фиксируется на 10px,
   поэтому все размеры здесь = rem × 10 и совпадают с вёрсткой один в один).
   ========================================================================== */
...
async function resolveFonts() {
  ...
  Object.keys(F).forEach(function (k) {
    var f = F[k];
    var dup = uniq.some(function (u) { return u.family === f.family && u.style === f.style; });
    if (!dup) uniq.push(f);
  });
...
function textNode(name, chars, font, size, fill, o) {
  ...
  else t.lineHeight = { value: 120, unit: 'PERCENT' };
  if (o.letterSpacing) t.letterSpacing = { value: o.letterSpacing, unit: 'PERCENT' };
  return t;
}

/* Квадратная заглушка фото: градиент + силуэт */
...
function buildSpeakers(F) {
  var CARD = (W - 160 - 5 * 20) / 6; // 276.67 — как в вёрстке: 6 колонок, gap 20

  var sec = frame('Блок · Спикеры', { mode: 'VERTICAL', gap: 48 });

  var h2 = textNode('Заголовок', 'Спикеры', F.black, 80, solid(C.blue), { lineHeight: 80 });
  sec.appendChild(h2);
...
/* ── Блок 2. Программа (табло) ───────────────────────────────────────── */
function programRow(item, F) {
  var row = frame('Строка · ' + item.time, {
    mode: 'HORIZONTAL', gap: 20, pv: 14, align: 'BASELINE'
  });
  ...
  var time = textNode('Время', item.time, F.medium, 16,
    solid(C.sky, item.brk ? 0.5 : 1), { lineHeight: 22 });
  ...
  if (item.who) {
    var from = item.topic.length;
    var to = chars.length;
    body.setRangeFontName(from, to, F.textReg);
    body.setRangeFontSize(from, to, 13);
    body.setRangeFills(from, to, [solid(C.white, 0.5)]);
  }
...
function buildProgram(F) {
  ...
  var h2 = textNode('Заголовок', 'Программа', F.black, 80, solid(C.blue), { lineHeight: 80 });
  sec.appendChild(h2);

  /* Лоток: светлая рамка вокруг тёмного ядра */
  ...
  var chipText = textNode('Подпись', 'ДЕЛОВАЯ ПРОГРАММА · 27.08 · MERCURY SPACE',
    F.medium, 12, solid(C.sky), { lineHeight: 12, letterSpacing: 14 });
  chip.appendChild(chipText);

  /* Две колонки по 9 строк */
...
async function build() {
  ...
  /* Перезапуск плагина обновляет макет, а не плодит копии */
  var old = figma.currentPage.findOne(function (n) { return n.type === 'FRAME' && n.name === NAME; });
  var at = old ? { x: old.x, y: old.y } : { x: 0, y: 0 };
  if (old) old.remove();

  var root = frame(NAME, {
    mode: 'VERTICAL', gap: 96, pt: 64, pb: 96, ph: 80, fill: solid(C.white)
  });
  ...
  var msg = F.missing
    ? 'Готово, но YS Geo / YS Text не найдены — подставлен Inter'
    : 'Готово: макет собран на YS Geo и YS Text';
  figma.closePlugin(msg);
}

build().catch(function (e) {
  figma.closePlugin('Ошибка: ' + (e && e.message ? e.message : String(e)));
});
