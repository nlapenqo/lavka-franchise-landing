...
// node tools/shot.mjs <url> <out.png> [--w 900] [--h 1000] [--block .business] [--off 30] [--click .sel] [--eval "js"] [--wait 800] [--print "js"]
//   --print — вычислить выражение после ожидания и вывести результат (замеры, тексты)
...
const [url, out] = args;
const W = +opt('w', 900), H = +opt('h', 1000), block = opt('block', ''), off = +opt('off', 30), click = opt('click', ''), evalJs = opt('eval', ''), wait = +opt('wait', 900), printJs = opt('print', '');
const mobile = W < 768;
...
try {
  ...
  await sleep(250);
  if (printJs) { const r = await send('Runtime.evaluate', { expression: printJs, returnByValue: true }); console.log(JSON.stringify(r.result.value)); }
